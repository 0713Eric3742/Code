...
if not os.path.exists('a.npy'):
    urllib.request.urlretrieve('https://storage.googleapis.com/quickdraw_dataset/full/numpy_bitmap/an.npy', 'a.npy')
if not os.path.exists('b.npy'):
    urllib.request.urlretrieve('https://storage.googleapis.com/quickdraw_dataset/full/numpy_bitmap/bn.npy', 'b.npy')
...
history = model2.fit(x=train_x, y=train_y, batch_size=256, epochs=100, verbose=1, validation_split=0.2)
plt.plot(history.history['acc'])
plt.plot(history.history['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
